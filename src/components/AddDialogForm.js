...
export default function AddDialogForm({ username, onChange, add }) {
    ...
    const handleAddNew = () => {
        ...
        fetch("http://localhost:8080/AI-Enabled_FinTech_B2B_Invoice_system/AddForm", config)
            .then((res) => res.json())
            .then(({ message }) => {
                if (message === "Success") {
                    handleClick();
                        ...
                        <Grid item xs={8}>
                            <TextField
                                ...
                                error={
                                    ...
                                    isNaN(parseInt(info.order_id))
                                }
                                helperText={
                                    info.order_id.length === 0 ||
                                    info.order_id.length === 6
                                        ? ""
